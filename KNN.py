    ...
    def predict_cluster(self, points, optimal):
        ...
        for point in points:
            if not optimal and self.k is not None:
                ...
                neighbours = self.get_k_neighbours(self.k, distances)
...
class Scene:
    ...
    def draw_description(self, clusters):
        ...
        if self.still_training:
            surf = font.render(f'Клавиши для рисования. [Q] - старт КНН, [R] рестарт',
                               False, colors[-1])
            screen.blit(surf, (3, 0))
        else:
            surf = font.render(f'КНН начато. [R] рестарт, ', False, colors[-1])
            screen.blit(surf, (3, 0))
